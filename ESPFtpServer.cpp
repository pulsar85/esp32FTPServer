 ...
 * modified to work with esp32 SD SPIFFS by Konstantin Bredihin (bredihin.ka@gmail.com)
 * 
...
void FtpServer::listDir(char * dir) {
	String fn, fs;
	File root = _fs.open(dir);
...
#ifdef FTP_DEBUG
		Serial.println(
				"Ftp server waiting for connection on port "
						+ String(FTP_CTRL_PORT));
#endif
		cmdStatus = 2;
...
boolean FtpServer::processCommand() {
...
#elif defined ESP32
			File root = _fs.open(cwdName);
			if (!root) {
				client.println("550 Can't open directory " + String(cwdName));
#ifdef FTP_DEBUG
				Serial.println(String(cwdName));
#endif
				// return;
			} else {
				// if(!root.isDirectory()){
	...
	//
		...
		if (!dataConnect())
			...
			while (file) {
#ifdef FTP_DEBUG
				Serial.println(String(String(file.name())));
#endif
				String fn, fs;
				if(file.isDirectory()) {
					...
					fn.remove(0, 1);
					//data.println( "+r,s <DIR> " + String(file.name()));
					data.println( "Type=dir;Perm=r;modify=20000101160656; " + fn);
					//data.println("Type=dir;Perm=r" + ";" + "modify=20000101160656;" + " " + String(file.name()));
					nm++;
#ifdef FTP_DEBUG
					Serial.print("  DIR : ");
					Serial.println(file.name());
#endif
					//		 	  listDir((char *)file.name());
					//if(levels){
					//	listDir(fs, file.name(), levels -1);
					//}
				} else {
...
#ifdef FTP_DEBUG
				Serial.println(
						"Renaming " + String(buf) + " to " + String(path));
#endif
